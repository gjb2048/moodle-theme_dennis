...
class theme_dennis_core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function dennisblocks($region, $blocksperrow = 0) {
        ...
        if (($this->page->blocks->region_has_content($displayregion, $this)) || ($editing)) {
            ...
            if ($editing) {
                ...
                $output .= html_writer::tag('span', html_writer::tag('span', get_string('region-'.$region, 'theme_dennis')),
                    array('class' => 'regionname col-12 text-center'));
                $output .= html_writer::end_tag('div');
            }
            if ($this->page->blocks->region_has_content($region, $this)) {
                if ($blocksperrow > 0) {
                    $regioncontent .= $this->dennis_blocks_for_region($region, $blocksperrow, $editing);
                } else {
    ...
    protected function dennis_blocks_for_region($region, $blocksperrow, $editing) {
